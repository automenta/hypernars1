...
const SystemView = () => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setHistory(prev => ({
                concepts: [...prev.concepts, nar.state.hypergraph.size].slice(-20),
                queue: [...prev.queue, nar.state.eventQueue.heap.length].slice(-20),
    ...
    const renderGraph = (data, label) => {
        ...
        const chars = [' ', '▂', '▃', '▄', '▅', '▆', '▇', '█'];
        const line = data.map(d => chars[Math.round((d / max) * (chars.length - 1))]).join('');
        return (
            <Box>
                <Text>{label.padEnd(12)}: {line} ({data[data.length - 1] || 0})</Text>
            </Box>
        );
    };
    ...
    return (
        <Box flexDirection="column" paddingX={1}>
            ...
            <Box flexDirection="column" marginTop={1}>
                ...
                {showConfig && nar && Object.entries(nar.config).map(([key, value]) => (
