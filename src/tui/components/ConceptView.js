...
const ConceptView = ({ conceptId, onClose }) => {
    ...
    const renderBeliefs = () => (
        <Box flexDirection="column" marginTop={1}>
            ...
            {showBeliefs &&
                concept.beliefs.slice(0, 5).map((belief) => (
                    <Box key={belief.id} flexDirection="column" marginLeft={2}>
                        <Text>
                            f:{belief.truth.f.toFixed(2)} c:
                            {belief.truth.c.toFixed(2)} | B: p:
                            {belief.budget.p.toFixed(2)} d:
                            {belief.budget.d.toFixed(2)} q:
                            {belief.budget.q.toFixed(2)}
                        </Text>
                        <Text color="gray">
                            Premises:{' '}
                            {belief.premises.length > 0
                                ? belief.premises
                                      .map((p) => p.substring(0, 10))
                                      .join(', ')
                                : 'None'}
                        </Text>
                    </Box>
    ...
    const renderRelated = () => (
        <Box flexDirection="column" marginTop={1}>
            ...
            {showRelated &&
                concept.args.map((argId) => (
                    <Box key={argId} marginLeft={2}>
                        <Text
                            color="cyan"
                            onClick={() => handleSelectConcept(argId)}
                        >
                            {argId}
                        </Text>
                    </Box>
    ...
    return (
        ...
        >
            ...
            <Text>
                Truth: f={concept.getTruth().f.toFixed(4)} c=
                {concept.getTruth().c.toFixed(4)}
            </Text>
            ...
            <Text>
                Press ESC to close. [b] to toggle beliefs. [r] to toggle
                related.
            </Text>
        </Box>
    );
