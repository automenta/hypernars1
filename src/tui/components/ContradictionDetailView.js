        ...
        {belief.evidence.length > 0 && (
            <Box flexDirection="column" marginLeft={2}>
                ...
                {belief.evidence.map((e, i) => (
                    <Text key={i}>- {e.source || 'unknown'} (strength: {e.strength.toFixed(2)})</Text>
                ))}
            </Box>
        )}
    </Box>
...
const ContradictionDetailView = ({contradictionId, onClose}) => {
    ...
    useEffect(() => {
        const analyzeContradiction = () => {
            ...
            if (nar.contradictionManager?.resolutionStrategies) {
                setStrategies(Object.keys(nar.contradictionManager.resolutionStrategies));
            }
        };
    ...
    useInput((input, key) => {
        if (key.escape) {
            ...
            setSelectedIndex(prev => Math.min(strategies.length - 1, prev + 1));
        } else if (key.return) {
            const selectedStrategy = strategies[selectedIndex];
            if (selectedStrategy) {
                const result = nar.contradictionManager.manualResolve(contradictionId, selectedStrategy);
                if (result) {
                    log(`Contradiction ${contradictionId} resolved with strategy: ${selectedStrategy}`);
                } else {
                    log(`Failed to resolve contradiction ${contradictionId} with strategy: ${selectedStrategy}`);
                }
                onClose();
            }
        }
    ...
    return (
        <Box flexDirection="column" padding={1} borderStyle={single} borderColor="red">
            ...
            <Text>(Press 'esc' to close, up/down to select strategy, Enter to resolve)</Text>
