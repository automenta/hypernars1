...
export class TruthValue {
    ...
    static revise(t1, t2) {
        ...
        const revisedFrequency =
            (t1.frequency * t1.priority + t2.frequency * t2.priority) /
            totalPriority;
    ...
    static transitive(t1, t2) {
        const f1 = t1.frequency,
            c1 = t1.confidence;
        const f2 = t2.frequency,
            c2 = t2.confidence;
        ...
        return new TruthValue(
            frequency,
            confidence,
            Math.min(t1.priority, t2.priority) * 0.8,
            doubt
        );
    }
    ...
    static induction(t1, t2) {
        ...
        return new TruthValue(
            frequency,
            confidence,
            Math.min(t1.priority, t2.priority) * 0.7,
            doubt
        );
    }
    ...
    static deduced(t1, t2) {
        ...
        return new TruthValue(
            frequency,
            confidence,
            Math.min(t1.priority, t2.priority),
            doubt
        );
    }
    ...
    equivalent(other, threshold = 0.01) {
        return (
            Math.abs(this.frequency - other.frequency) < threshold &&
