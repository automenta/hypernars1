...
describe('ExplanationSystem', () => {
    ...
    it('should identify transitive inheritance', () => {
        ...
        const conclusion = nar.inheritance('A', 'C', {
            premises: [p1, p2],
        ...
        expect(rootNode.premises).toHaveLength(2);
    });
    ...
    it('should identify analogy', () => {
        ...
        // Manually create the conclusion, passing the derivation rule name
        const conclusion = nar.inheritance('B', 'C', {
            premises: [p1, p2],
            derivedBy: 'analogy',
        });
    ...
    it('should fallback to "assertion" for beliefs with no known derivation rule', () => {
        ...
        const conclusion = nar.implication('premise1', 'conclusion', {
            premises: [p1],
        });
    ...
    it('should explain a derived temporal relation using templates', () => {
        ...
        expect(explanation).toContain(
            'CONCLUSION: TemporalRelation(TimeInterval(event_A, 1000, 2000), TimeInterval(event_B, 3000, 4000), before)'
        );
    ...
    describe('Explanation Formats', () => {
        ...
        beforeEach(() => {
            ...
            conclusionId = nar.inheritance('A', 'C', {
                premises: [p1, p2],
                derivedBy: 'transitivity',
            });
        });
        ...
        it('should generate a detailed explanation', () => {
            ...
            expect(explanation).toContain(
                '- Because Inheritance(A, B) and Inheritance(B, C), it follows through transitivity that Inheritance(A, C)'
            );
            expect(explanation).toContain(
                '  - It is a direct assertion that Inheritance(A, B)'
            );
            expect(explanation).toContain(
                '  - It is a direct assertion that Inheritance(B, C)'
            );
        });
        ...
        it('should generate a story explanation', () => {
            ...
            expect(explanation).toContain(
                'how I came to believe that A is a kind of C'
            );
    ...
    describe('Advanced Explanation Formats', () => {
        ...
        it('should generate a justification with supporting and conflicting evidence', () => {
            ...
            nar.api.inheritance('penguin', 'bird', {
            ...
            expect(explanation).toContain(
                'Justification for: Inheritance(penguin, bird)'
            );
            expect(explanation).toContain(
                'Supporting Evidence: This appears to be a base assertion.'
            );
            expect(explanation).toContain(
                'Conflicting Evidence (overridden or merged):'
            );
            expect(explanation).toContain(
                'An alternative belief exists with confidence'
            );
        });
    });
    ...
    describe('Contradiction Explanations', () => {
        it('should include a note about a resolved contradiction in the justification', () => {
            ...
            const explanation = nar.explain(conceptId, {
                format: 'justification',
            });
            expect(explanation).toContain(
                "Note: This belief was part of a contradiction resolved via the 'merge' strategy."
            );
        });
    });
});
