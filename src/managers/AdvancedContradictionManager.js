...
export class AdvancedContradictionManager extends ContradictionManagerBase {
    constructor(nar) {
        ...
        this.config = {
            ...defaultConfig,
            ...nar.config.advancedContradictionManager,
        };
    ...
    contradict(belief1Id, belief2Id, { strength = 0.7, context = null } = {}) {
        ...
        if (!belief1 || !belief2) {
            this.nar.emit('log', {
                message:
                    'Cannot create contradiction: one or both beliefs not found.',
                level: 'warn',
            });
            return null;
        }
        ...
        const contradictionId = this.nar.api.id('Contradiction', [
            belief1Id,
            belief2Id,
            context,
        ]);
    ...
    detectContradiction(hyperedgeId) {
        ...
        for (let i = 0; i < hyperedge.beliefs.length; i++) {
            for (let j = i + 1; j < hyperedge.beliefs.length; j++) {
                if (
                    this._areContradictory(
                        hyperedge.beliefs[i].truth,
                        hyperedge.beliefs[j].truth
                    )
                ) {
                    contradictoryPairs.push({
                        belief1: hyperedge.beliefs[i],
                        belief2: hyperedge.beliefs[j],
                        severity: this._contradictionSeverity(
                            hyperedge.beliefs[i].truth,
                            hyperedge.beliefs[j].truth
                        ),
                    });
    ...
    detectAndResolveInterEdgeContradictions(hyperedge) {
        ...
        for (const [
            otherHyperedgeId,
            otherHyperedge,
        ] of this.nar.state.hypergraph.entries()) {
            if (
                otherHyperedge.type === 'Inheritance' &&
                otherHyperedge.id !== hyperedge.id &&
                otherHyperedge.args[0] === subjectId
            ) {
                const otherPredicateId = otherHyperedge.args[1];

                if (
                    predicateId === `Negation(${otherPredicateId})` ||
                    otherPredicateId === `Negation(${predicateId})`
                ) {
                    const belief1 = hyperedge.getStrongestBelief();
                    const belief2 = otherHyperedge.getStrongestBelief();

                    if (belief1 && belief2) {
                        const revisedTruth = TruthValue.revise(
                            belief1.truth,
                            belief2.truth.negate()
                        );
                        const revisedBudget = belief1.budget.merge(
                            belief2.budget
                        );
                        ...
                        otherHyperedge.beliefs[0].budget =
                            otherHyperedge.beliefs[0].budget.scale(
                                this.config.interEdgeBudgetScale
                            );
                    }
                }
            }
    ...
    resolveContradictions() {
        ...
        if (now < this.circuitBreaker.openUntil) {
            this.nar.emit('log', {
                message: 'Contradiction resolution circuit breaker is open.',
                level: 'warn',
            });
            return;
        }
        ...
        for (const hyperedgeId of contradictionsToResolve) {
            ...
            if (contradictionData && !contradictionData.resolved) {
                const strategyName = this._selectResolutionStrategy(
                    hyperedgeId,
                    contradictionData
                );
                const resolution = this.manualResolve(
                    hyperedgeId,
                    strategyName
                );
                if (!resolution) {
        ...
        if (failuresThisRun > 0) {
            ...
            if (this.circuitBreaker.failures >= this.circuitBreaker.threshold) {
                this.circuitBreaker.openUntil =
                    now + this.circuitBreaker.duration;
                this.nar.emit('log', {
    ...
    manualResolve(hyperedgeId, strategyName, customParams = {}) {
        ...
        const strategy =
            this.resolutionStrategies[strategyName] ||
    ...
    _resolveByDominantEvidence(hyperedgeId, contradiction) {
        const hyperedge = this.nar.state.hypergraph.get(hyperedgeId);
        const beliefsWithEvidence = hyperedge.beliefs
            .map((belief) => ({
                belief,
                evidenceStrength: this._calculateEvidenceStrength(
                    hyperedgeId,
                    belief
                ),
            }))
        ...
        const modifiedBeliefs = otherBeliefs.map((item) => {
            ...
            modifiedBelief.truth = new TruthValue(
                ...
                modifiedBelief.truth.confidence *
                    this.config.weakenConfidenceFactor,
                modifiedBelief.truth.priority *
                    this.config.weakenPriorityFactor,
                Math.min(
                    1.0,
                    (modifiedBelief.truth.doubt || 0) +
                        this.config.weakenDoubtFactor
                )
            );
    ...
    _resolveByMerging(hyperedgeId, contradiction) {
        ...
        const mergedTruth = TruthValue.revise(
            belief1.belief.truth,
            belief2.belief.truth
        );
        mergedTruth.doubt = Math.min(
            1.0,
            (mergedTruth.doubt + this.config.mergeDoubtPenalty) * 0.7
        );

        const mergedBudget = belief1.belief.budget
            .merge(belief2.belief.budget)
            .scale(this.config.mergeBudgetPenalty);
    ...
    _resolveBySpecialization(hyperedgeId, contradiction) {
        ...
        const context =
            this._getBeliefContext(hyperedge, conflictingBelief) ||
            'alternative_context';

        const specializationResult = this._createContextualSpecialization(
            hyperedge,
            conflictingBelief,
            context
        );
        hyperedge.beliefs = hyperedge.beliefs.filter(
            (b) => b !== conflictingBelief
        );
    ...
    _evidenceWeightedResolution(hyperedgeId, contradiction) {
        ...
        if (totalWeight > 0) {
            ...
            hyperedge.beliefs = [
                { truth: newTruth, budget: newBudget, timestamp: Date.now() },
            ];
    ...
    _recencyBiasedResolution(hyperedgeId, contradiction) {
        const hyperedge = this.nar.state.hypergraph.get(hyperedgeId);
        const mostRecent = [...hyperedge.beliefs].sort(
    ...
    _sourceReliabilityResolution(
        ...
        hyperedge.beliefs.forEach((belief) => {
            ...
            const sourceReliability =
                ...
                this.nar.state.sourceReliability?.get(source) ||
                this.config.defaultSourceReliability;
        ...
        if (totalWeight > 0) {
            ...
            hyperedge.beliefs = [
                {
                    truth: newTruth,
                    budget: Budget.full().scale(
                        this.config.sourceReliabilityBudgetScale
                    ),
                    timestamp: Date.now(),
                },
            ];
    ...
    _hasReliableSourceDifference(strongest, nextStrongest) {
        ...
        const reliability1 =
            this.nar.state.sourceReliability?.get(source1) ||
            this.config.defaultSourceReliability;
        const reliability2 =
            this.nar.state.sourceReliability?.get(source2) ||
            this.config.defaultSourceReliability;

        return (
            Math.abs(reliability1 - reliability2) >
            this.config.sourceReliabilityDifferenceThreshold
        );
    }
    ...
    _shouldUseEvidenceWeighting(hyperedge) {
        return (
            hyperedge.evidence &&
            hyperedge.evidence.length > this.config.evidenceWeightingThreshold
        );
    }
    ...
    _hasDominantEvidence(strongest, nextStrongest) {
        return (
            ...
            nextStrongest.evidenceStrength * this.config.dominantEvidenceFactor
        );
    }

    _hasDistinctContexts(hyperedge, strongest, nextStrongest) {
        const context1 = this._getBeliefContext(hyperedge, strongest.belief);
        const context2 = this._getBeliefContext(
            hyperedge,
            nextStrongest.belief
        );
    ...
    _calculateEvidenceStrength(hyperedgeId, belief) {
        ...
        const intrinsicWeight =
            this.nar.config.intrinsicStrengthWeight ||
            this.config.intrinsicStrengthWeight;
        const evidenceWeight =
            this.nar.config.evidenceStrengthWeight ||
            this.config.evidenceStrengthWeight;
        const sourceReliabilityWeight =
            this.nar.config.sourceReliabilityWeight ||
            this.config.sourceReliabilityWeight;
        const recencyWeight =
            this.nar.config.recencyWeight || this.config.recencyWeight;

        const now = Date.now();
        const evidenceList =
            hyperedge.evidence?.filter((e) => e.beliefId === belief.id) || [];

        const totalEvidenceStrength = evidenceList.reduce((sum, e) => {
            const ageInSeconds = (now - e.timestamp) / 1000;
            const decay = Math.exp(
                -ageInSeconds /
                    (this.nar.config.temporalHorizon *
                        this.config.evidenceDecayHours)
            );
        ...
        const sourceReliability =
            evidenceList.reduce((sum, e) => {
                const reliability =
                    this.nar.state.sourceReliability?.get(e.source) ||
                    this.config.defaultSourceReliability;
        ...
        const intrinsicStrength =
            belief.truth.confidence * belief.budget.priority;

        const finalStrength =
            intrinsicStrength * intrinsicWeight +
    ...
    _createContextualSpecialization(
        originalHyperedge,
        conflictingBelief,
        context
    ) {
        ...
        const specializedTruth = new TruthValue(
            ...
            conflictingBelief.truth.confidence *
                this.config.specializationConfidenceBoost,
            conflictingBelief.truth.priority,
            (conflictingBelief.truth.doubt || 0) *
                this.config.specializationDoubtReduction
        );

        const specializedBudget = conflictingBelief.budget.scale(
            this.config.specializationBudgetBoost
        );
        ...
        const specialization = new Hyperedge(
            this.nar,
            newId,
            originalHyperedge.type,
            newArgs
        );
        ...
        const similarityFreq = Math.max(
            0.1,
            1.0 - conflictingBelief.truth.confidence
        );
    ...
    _getBeliefContext(hyperedge, belief) {
        ...
        evidenceList.forEach((e) => {
            if (e.source) {
                sourceCounts.set(
                    e.source,
                    (sourceCounts.get(e.source) || 0) + 1
                );
            }
        });

        if (sourceCounts.size > 0) {
            return [...sourceCounts.entries()].sort(
                (a, b) => b[1] - a[1]
            )[0][0];
        }
    ...
    _areContradictory(truth1, truth2) {
        ...
        const isStrongContradiction =
            freqDiff >
                (this.nar.config.contradictionThreshold ||
                    this.config.strongContradictionThreshold) &&
            avgConfidence > this.config.strongContradictionConfidence;

        const isModerateContradiction =
            freqDiff > this.config.moderateContradictionFreqDiff &&
            confDiff > this.config.moderateContradictionConfDiff &&
            avgConfidence > this.config.moderateContradictionAvgConfidence;
    ...
    analyze(hyperedgeId) {
        ...
        if (!hyperedge || !hyperedge.beliefs || hyperedge.beliefs.length < 2) {
            return {
                message:
                    'No significant contradiction found for this hyperedge.',
                contradictions: [],
                resolutionSuggestion: null,
            };
        }

        const beliefsWithAnalysis = hyperedge.beliefs
            .map((b, i) => ({
                index: i,
                belief: b,
                truth: b.truth,
                ...
                evidence: hyperedge.evidence
                    ? hyperedge.evidence.filter((e) => e.beliefId === b.id)
                    : [],
                evidenceStrength: this._calculateEvidenceStrength(
                    hyperedgeId,
                    b
        ...
        const strategy = this._selectResolutionStrategy(hyperedgeId, {
            pairs: [],
        });
        const suggestionDetails = this._getResolutionSuggestionDetails(
            hyperedgeId,
            strategy
        );
        ...
        return {
            ...
            resolutionSuggestion: {
                ...
                confidence:
                    Math.max(
                        ...beliefsWithAnalysis.map((b) => b.evidenceStrength)
                    ) -
                    Math.min(
                        ...beliefsWithAnalysis.map((b) => b.evidenceStrength)
                    ),
