...
export class TemporalReasoner extends TemporalManagerBase {
    ...
    addConstraint(event1, event2, relation, options = {}) {
        const constraintId = id('TemporalConstraint', [
            event1,
            event2,
            relation,
        ]);
        ...
        const hyperedgeId = this.nar.api.addHyperedge(
            'TemporalRelation',
    ...
    relate(term1, term2, relation, options = {}) {
        const interval1 = Array.from(this.intervals.values()).find(
            (i) => i.term === term1
        );
        const interval2 = Array.from(this.intervals.values()).find(
            (i) => i.term === term2
        );
    ...
    during(eventTerm, start, end, options = {}, now) {
        ...
        if (typeof end === 'string' && patterns.includes(end.toLowerCase())) {
            return this._createRecurringInterval(
                eventTerm,
                start,
                end.toLowerCase(),
                options,
    ...
    _createRecurringInterval(
        ...
        const end =
            start + this.config.recurringIntervalDurationHours * 60 * 60 * 1000;
    ...
    query(subject, constraints = {}) {
        ...
        this.intervals.forEach((interval) => {
            if (interval.term === subject) {
                let matches = true;
                if (constraints.before && interval.end >= constraints.before)
                    matches = false;
                if (constraints.after && interval.start <= constraints.after)
                    matches = false;
    ...
    describeTemporalRelationship(event1, event2) {
        ...
        if (!relationship)
            return `No known temporal relationship between ${event1} and ${event2}`;

        const rel = relationship.relation
            .replace(/([A-Z])/g, ' $1')
            .toLowerCase();
    ...
    _propagateConstraint(initialConstraint) {
        ...
        const maxIterations =
            this.temporalConstraints.size +
            this.config.maxPropagationIterations;
        let iterations = 0;
        ...
        while (propagationQueue.length > 0 && iterations < maxIterations) {
            ...
            this.temporalConstraints.forEach((existingConstraint) => {
                if (newConstraint.event2 === existingConstraint.event1) {
                    const composedRelation = this._composeRelationships(
                        newConstraint.relation,
                        existingConstraint.relation
                    );
                    if (composedRelation) {
                        const composedTruth = this._composeTruthValues(
                            newConstraint,
                            existingConstraint
                        );
            ...
            inferredRelations.forEach((inferred) => {
                const existing = this.inferRelationship(
                    inferred.event1,
                    inferred.event2
                );
                if (
                    existing &&
                    JSON.stringify(existing.relation) ===
                        JSON.stringify(inferred.relation)
                ) {
                    return;
                }

                const newId = this.nar.api.addHyperedge(
                    'TemporalRelation',
                    [inferred.event1, inferred.event2, inferred.relation],
                    {
        ...
        if (iterations >= maxIterations && maxIterations > 10) {
            this.nar.emit('log', {
                message: 'Temporal propagation reached max iterations.',
                level: 'warn',
            });
        }
    }
    ...
    _composeTruthValues(constraint1, constraint2) {
        const t1 =
            constraint1.truth ||
            this.nar.state.hypergraph.get(constraint1.id)?.getTruth() ||
            TruthValue.certain();
        const t2 =
            constraint2.truth ||
            this.nar.state.hypergraph.get(constraint2.id)?.getTruth() ||
            TruthValue.certain();
    ...
    _composePath(path) {
        ...
        for (let i = 1; i < path.length; i++) {
            composedRelation = this._composeRelationships(
                composedRelation,
                path[i].relation
            );
            if (!composedRelation) {
    ...
    queryTimeWindow(start, end, options = {}) {
        ...
        for (const interval of this.intervals.values()) {
            if (
                interval.overlapsWith(start, end) &&
                interval.truth.confidence >= minConfidence
            ) {
                results.push({
                    id: interval.id,
                    term: interval.term,
    ...
    _wouldCreateContradiction(event1, event2, newRelation) {
        ...
        if (
            !existing ||
            !existing.relation ||
            existing.relation === 'ambiguous'
        ) {
            return false;
        }

        const existingRelations = Array.isArray(existing.relation)
            ? existing.relation
            : [existing.relation];
        const newRelations = Array.isArray(newRelation)
            ? newRelation
            : [newRelation];
        ...
        const composed = this._composeRelationships(
            existing.relation,
            this._getInverseTemporalRelation(newRelation)
        );
    ...
    adjustTemporalHorizon() {
        ...
        const newHorizon = Math.min(
            maxHorizon,
            baseHorizon + Math.floor(Math.sqrt(numConstraints))
        );
    ...
    predict(term, pattern, horizonInMinutes) {
        ...
        for (const constraint of this.temporalConstraints.values()) {
            ...
            if (constraint.event1 === term) {
                ...
                relation = this._getInverseTemporalRelation(
                    constraint.relation
                );
            ...
            if (
                ['before', 'meets', 'overlaps', 'starts', 'during'].includes(
                    relation
                )
            ) {
                const sourceInterval = Array.from(this.intervals.values()).find(
                    (i) => i.term === sourceEvent && i.end >= now
                );
                ...
                if (sourceInterval) {
                    ...
                    if (
                        ...
                        const predictedHyperedge =
                            this.nar.state.hypergraph.get(
                                id('Term', [predictedEvent])
                            );
                        if (
                            !predictions.has(predictedEvent) ||
                            predictions.get(predictedEvent).confidence <
                                confidence
                        ) {
                            predictions.set(predictedEvent, {
                                term: predictedEvent,
                                confidence,
                                truth: predictedHyperedge
                                    ? predictedHyperedge.getTruth()
                                    : TruthValue.unknown(),
        ...
        for (const interval of this.intervals.values()) {
            if (interval.start <= now && interval.end > futureTime) {
                ...
                if (
                    !predictions.has(interval.term) ||
                    predictions.get(interval.term).confidence < confidence
                ) {
    ...
    _calculatePredictionConfidence(
        ...
        let baseConfidence =
            this.config.predictionBaseConfidence[relation] ||
            this.config.predictionBaseConfidence.default;

        const sourceConfidence =
            sourceInterval.truth?.confidence ??
            this.config.defaultIntervalConfidence;
