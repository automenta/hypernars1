export class ExplanationSystem {
    constructor(nar) {
        ...
        this.explanationTemplates = {
            ...
            transitivity:
                'Because {premise1} and {premise2}, it follows through transitivity that {conclusion}.',
            analogy:
                'By analogy, since {premise1} and {premise2}, it is likely that {conclusion}.',
            induction:
                'Observing that {premise1} and {premise2} are related, we can induce that {conclusion}.',
            derived:
                'From the premise {premise1}, it was derived that {conclusion}.',
            default:
                'It is believed that {conclusion}, based on the premise {premise1}.',
        };
    }
    ...
    _formatByTarget(format, hyperedgeId, path, options) {
        switch (format) {
            ...
            case 'concise': {
                ...
                return flatPath
                    .map((step) => this._formatHyperedge(step))
                    .join(' -> ');
            }
            case 'technical':
    ...
    _formatCausalExplanation(path) {
        ...
        const reasoningChain = path
            ...
            .map(
                (step, i) =>
                    `${'  '.repeat(i)}-> Because of the belief that ${this._formatHyperedge(step)}.`
            );
    ...
    _formatCounterfactualExplanation(hyperedgeId, options) {
        ...
        const premiseToReplace = this._findPremiseToReplace(
            hyperedgeId,
            options.depth
        );
        if (!premiseToReplace) {
        ...
        const sandbox = this._runCounterfactualSandbox(
            premiseToReplace,
            alternative
        );
        return this._analyzeCounterfactualResults(
            originalHyperedge,
            sandbox,
            alternative,
            premiseToReplace
    ...
    _runCounterfactualSandbox(premiseToReplace, alternative) {
        ...
        const truthValue = premiseToReplace.getStrongestBelief()?.truth || {
            frequency: 1.0,
            confidence: 0.9,
        };
    ...
    _analyzeCounterfactualResults(
        originalHyperedge,
        sandbox,
        alternative,
        premiseToReplace
    ) {
        const conclusionInSandbox = sandbox.api.getBeliefs(
            originalHyperedge.id
        );
        const explanation = [
        ...
        if (conclusionInSandbox.length > 0) {
            ...
            explanation.push(
                `1.  The original conclusion **still holds**, but its confidence would change to ${this._formatConfidence(newTruth)}.`
            );
        } else {
            explanation.push(
                `1.  The original conclusion **no longer holds**.`
            );
        }

        const newConclusions = this._findNewConclusions(
            sandbox,
            new Set([...this.nar.state.hypergraph.keys()]),
            premiseToReplace.id,
            alternative
        );
        if (newConclusions.length > 0) {
            explanation.push(
                `2.  The following **new conclusions** would become prominent:`
            );
            newConclusions.forEach((c) => {
                explanation.push(
                    `    - ${this._formatHyperedge(c)} (Confidence: ${this._formatConfidence(c.getTruth())})`
                );
            });
        } else {
            explanation.push(
                '2.  No other significant new conclusions would be reached in the simulation.'
            );
        }
    ...
    _formatJustification(hyperedgeId) {
        ...
        explanation.push(
            this._getConflictingEvidence(hyperedge, strongestBelief)
        );
    ...
    _getSupportingEvidence(hyperedgeId) {
        ...
        const evidenceLines = supportingPremises.map(
            (premise) =>
                `  - Because of: ${this._formatHyperedge(premise)} (Confidence: ${this._formatConfidence(premise.truth)})`
        );
        return [
            'Supporting Evidence (from derivation):',
            ...evidenceLines,
        ].join('\n');
    }

    _getConflictingEvidence(hyperedge, strongestBelief) {
        const conflictingBeliefs = hyperedge.beliefs.filter(
            (b) => b.id !== strongestBelief.id
        );
        if (conflictingBeliefs.length === 0) return null;

        const evidenceLines = conflictingBeliefs.map(
            (belief) =>
    ...
    _getContradictionInfo(hyperedgeId) {
        const contradiction =
            this.nar.contradictionManager.contradictions.get(hyperedgeId);
    ...
    _formatDetailedExplanation(hyperedgeId, path, options) {
        ...
        return [
            ...
            includeConfidence
                ? `Confidence: ${this._formatConfidence(conclusion.truth)}\n`
                : null,
            'REASONING PATH:',
            this._formatExplanationTree(conclusion, 0),
            this._getAlternativePerspectives(
                hyperedge,
                hyperedgeId,
                maxAlternatives
            ),
    ...
    _formatExplanationTree(node, level) {
        ...
        const template =
            this.explanationTemplates[rule] ||
            this.explanationTemplates['default'];

        const premise1 = node.premises?.[0]
            ? this._formatHyperedge(node.premises[0])
            : 'a previous belief';
        const premise2 = node.premises?.[1]
            ? this._formatHyperedge(node.premises[1])
            : 'another belief';
        ...
        const childExplanations = (node.premises || [])
            .map((premiseNode) =>
                this._formatExplanationTree(premiseNode, level + 1)
            )
            .join('');
    ...
    _generateStoryExplanation(rootNode) {
        ...
        const premiseStories = (conclusion.premises || []).map((p) =>
            this._formatTermForStory(p)
        );
        const premiseText =
            premiseStories.length > 0
    ...
    _formatHyperedgeSimple(hyperedge) {
        if (!hyperedge) return 'an unknown concept';
        const args = hyperedge.args
            ? hyperedge.args
                  .map((arg) => arg.replace(/Term\((.*?)\)/g, '$1'))
                  .join(', ')
            : '';
        return `${hyperedge.type}(${args})`;
    }
    ...
    _formatConfidence(truth) {
        ...
        if (confidence >= 0.9)
            description = `Very High (${(confidence * 100).toFixed(0)}%)`;
        else if (confidence >= 0.75)
            description = `High (${(confidence * 100).toFixed(0)}%)`;
        else if (confidence >= 0.5)
            description = `Moderate (${(confidence * 100).toFixed(0)}%)`;
        else if (confidence >= 0.25)
            description = `Low (${(confidence * 100).toFixed(0)}%)`;
