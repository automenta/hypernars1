...
const createMockHyperedge = (
    ...
    const edge = new Hyperedge(type, args, {
        truth,
        budget,
        premises,
        derivedBy,
    ...
    edge.getStrongestBelief = jest
        .fn()
        .mockReturnValue({ truth, budget, premises, derivedBy });
...
describe('AdvancedLearningEngine', () => {
    ...
    describe('Experience Handling', () => {
        it('should record an experience and add it to the buffer', () => {
            ...
            expect(engine.experienceBuffer).toHaveLength(1);
        ...
        it('should prune the experience buffer when it exceeds max size', () => {
            ...
            expect(engine.experienceBuffer).toHaveLength(5);
    ...
    describe('Learning Mechanisms', () => {
        it('should analyze failure and penalize the responsible rule and premise', () => {
            const premiseEdge = createMockHyperedge(
                'premise1',
                'Term',
                ['A'],
            ...
            expect(
                premiseEdge.getStrongestBelief().truth.confidence
            ).toBeLessThan(0.8);
        });
        ...
        it('should learn action-consequence mappings', () => {
            ...
            const expectedId = id('ActionConsequence', [
                'action1',
                'consequence1',
            ]);
    ...
    describe('Rule Adaptation', () => {
        it('should disable a rule if its effectiveness is below the threshold', () => {
            ...
            engine.ruleProductivity.set('BadRule', {
                successes: 0,
                attempts: 10,
            });
            ...
            expect(mockNar.derivationEngine.rules.get('BadRule').enabled).toBe(
                false
            );
            expect(mockNar.emit).toHaveBeenCalledWith(
                'rule-disabled',
                expect.any(Object)
            );
        });
        ...
        it('should re-enable a rule if its effectiveness improves', () => {
            ...
            engine.ruleProductivity.set('GoodRule', {
                successes: 8,
                attempts: 10,
            });
            ...
            expect(mockNar.derivationEngine.rules.get('GoodRule').enabled).toBe(
                true
            );
            expect(mockNar.emit).toHaveBeenCalledWith(
                'rule-enabled',
    ...
    describe('Pattern Discovery and Concept Formation', () => {
        ...
        it('should form a new concept from a frequent pattern', () => {
            ...
            jest.spyOn(engine, '_getTermsFromSignature').mockReturnValue([
                'TypeA',
                'TypeB',
            ]);
