...
export class AdvancedDerivationEngine extends DerivationEngineBase {
    constructor(nar) {
        ...
        this._deriveTransitiveInheritance =
            this._deriveTransitiveInheritance.bind(this);
    ...
    evaluateRules() {
        ...
        for (const [ruleName, rule] of this.rules) {
            const ruleStats = stats.get(ruleName);
            if (ruleStats && ruleStats.attempts > 5) {
                ...
                rule.priority =
                    rule.successRate * 0.7 + rule.applicability * 0.3;
                rulesChanged = true;
            } else {
                rule.priority *= 0.995;
            }
    ...
    _registerDefaultRules() {
        this.registerRule(
            'Inheritance',
            (event) =>
                this.nar.state.hypergraph.get(event.target)?.type ===
                'Inheritance',
            (h, e, r) => this._deriveInheritance(h, e, r)
        );
        this.registerRule(
            'Similarity',
            (event) =>
                this.nar.state.hypergraph.get(event.target)?.type ===
                'Similarity',
            (h, e, r) => this._deriveSimilarity(h, e, r)
        );
        this.registerRule(
            'Implication',
            (event) =>
                this.nar.state.hypergraph.get(event.target)?.type ===
                'Implication',
            (h, e, r) => this._deriveImplication(h, e, r)
        );
        this.registerRule(
            'Equivalence',
            (event) =>
                this.nar.state.hypergraph.get(event.target)?.type ===
                'Equivalence',
            (h, e, r) => this._deriveEquivalence(h, e, r)
        );
        this.registerRule(
            'Conjunction',
            (event) =>
                this.nar.state.hypergraph.get(event.target)?.type ===
                'Conjunction',
            (h, e, r) => this._deriveConjunction(h, e, r)
        );
        this.registerRule(
            'TemporalRelation',
            (event) =>
                this.nar.state.hypergraph.get(event.target)?.type ===
                'TemporalRelation',
            (h, e, r) => this._deriveTransitiveTemporalRelation(h, e, r)
        );
    }
    ...
    applyDerivationRules(event) {
        if (event.type === 'add-belief') {
            ...
            if (hyperedge) {
                ...
                this.nar.questionHandler.checkQuestionAnswers(
                    hyperedge.id,
                    newBelief
                );
            }
        ...
        if (
            !hyperedge ||
            activation <= this.nar.config.inferenceThreshold ||
            pathLength > this.nar.config.maxDerivationDepth
        )
            return;

        const activeRules = [...this.rules.values()].filter(
            (rule) => rule.enabled !== false && rule.condition(event)
        );

        if (activeRules.length === 0) return;

        const weightedRules = activeRules.map((rule) => {
            const ruleName = [...this.rules.entries()].find(
                ([name, r]) => r === rule
            )?.[0];
            const dynamicFactor =
                this.nar.cognitiveExecutive.getRulePriority(ruleName);
            return { rule, weight: rule.priority * dynamicFactor };
        });

        const totalPriority = weightedRules.reduce(
        ...
        if (selectedRule) {
            for (const [name, ruleObject] of this.rules.entries()) {
                if (ruleObject === selectedRule) {
                    ...
                    this.nar.cognitiveExecutive.monitorDerivation(
                        name,
                        success,
                        computationalCost,
                        value
    ...
    _deriveInheritance({ args: [subject, predicate] }, event, ruleName) {
        ...
        const currentHyperedge = this.nar.state.hypergraph.get(
            id('Inheritance', [subject, predicate])
        );

        (this.nar.state.index.byArg.get(predicateId) || new Set()).forEach(
            (termId) => {
                const forwardChainEdge = this.nar.state.hypergraph.get(termId);
                if (
                    forwardChainEdge?.type === 'Inheritance' &&
                    getArgId(forwardChainEdge.args[0]) === predicateId
                ) {
    ...
    _deriveTransitiveInheritance(context, event) {
        ...
        const { pathHash, pathLength } =
            event;

        const subjectId = getArgId(subject);
        const predicateId = getArgId(predicate);
        const key = this._memoKey(
            'Inheritance',
            [subjectId, predicateId],
            pathHash
        );
        if (
            this.nar.state.memoization.has(key) &&
            this.nar.state.memoization.get(key) <= pathLength
        )
            return;
        ...
        (this.nar.state.index.byArg.get(subjectId) || new Set()).forEach(
            (termId) => {
                const backwardChainEdge = this.nar.state.hypergraph.get(termId);
                if (
                    backwardChainEdge?.type === 'Inheritance' &&
                    getArgId(backwardChainEdge.args[1]) === subjectId
                ) {
        ...
        this._derivePropertyInheritance(
            subject,
            predicateId,
            ruleName
        );

        this._findAndDeriveInductionFromInheritance(
            subject,
            predicateId,
            ruleName,
            event
    ...
    _derivePropertyInheritance(
        subject,
        predicateId
    ) {
        if (
            this.nar.state.hypergraph.has(id('Instance', [subject, 'entity']))
        ) {
            (this.nar.state.index.byArg.get(predicateId) || new Set()).forEach(
                (propId) => {
    ...
    _findAndDeriveInductionFromInheritance(
        subject,
        predicateId,
        ruleName,
        event
        ...
        (this.nar.state.index.byArg.get(predicateId) || new Set()).forEach(
            (termId) => {
                ...
                if (
                    other?.type === 'Inheritance' &&
                    getArgId(other.args[1]) === predicateId &&
                    getArgId(other.args[0]) !== subject
                ) {
                    const context = {
                        term1: subject,
                        term2: other.args[0],
                        predicate: predicateId,
                        premise1: this.nar.state.hypergraph.get(
                            id('Inheritance', [subject, predicateId])
                        ),
                        premise2: other,
                        ruleName,
                    };
    ...
    _deriveSimilarity({ args: [term1, term2] }, event, ruleName) {
        ...
        const { activation, budget, pathHash, pathLength, derivationPath } =
            event;
        ...
        (this.nar.state.index.byArg.get(term1Id) || new Set()).forEach(
            (termId) => {
                const premise = this.nar.state.hypergraph.get(termId);
                if (
                    premise?.type === 'Inheritance' &&
                    getArgId(premise.args[0]) === term1Id
                ) {
                    const context = {
                        term1,
                        term2,
                        predicate: premise.args[1],
                        similarity: this.nar.state.hypergraph.get(
                            id('Similarity', [term1, term2])
                        ),
                        premise,
                        ruleName,
                    };
    ...
    _performInductiveSimilarityDerivation(context, event) {
        ...
        const truth = TruthValue.induction(
            premise1.getTruth(),
            premise2.getTruth()
        );
    ...
    _deriveAnalogy(context, event) {
        const { term1, term2, predicate, similarity, premise, ruleName } =
            context;
        const { budget, pathHash, pathLength } = event;
        ...
        const truth = TruthValue.analogy(
            similarity.getTruth(),
            premise.getTruth()
        );
    ...
    _deriveImplication({ args: [premise, conclusion] }, event, ruleName) {
        ...
        if (this.nar.state.hypergraph.has(premiseId)) {
            ...
            if (typeof targetConclusion === 'string') {
                targetConclusion =
                    this.nar.expressionEvaluator.parse(targetConclusion);
            }
    ...
    _deriveTransitiveTemporalRelation(
        { args: [premise, conclusion, relation] },
        event,
        ruleName
    ) {
        const premiseId = getArgId(premise);
        ...
        (this.nar.state.index.byArg.get(premiseId) || new Set()).forEach(
            (termId) => {
                const middle = this.nar.state.hypergraph.get(termId);

                if (
                    middle?.type === 'TemporalRelation' &&
                    getArgId(middle.args[1]) === premiseId
                ) {
                    const firstTerm = middle.args[0];
                    const firstRelation = middle.args[2];

                    const composedRelations = this._composeTemporalRelations(
                        firstRelation,
                        relation
                    );
                    if (composedRelations) {
                        composedRelations.forEach((newRelation) => {
                            this.nar.api.addHyperedge(
                                'TemporalRelation',
                                [firstTerm, conclusion, newRelation],
                                {
                                    truth: TruthValue.transitive(
                                        middle.getTruth(),
                                        eventHyperedge.getTruth()
                                    ),
    ...
    _composeTemporalRelations(rel1, rel2, triedInverse = false) {
        const table = {
            ...
            overlaps: {
                ...
                starts: ['overlaps'],
                during: ['during', 'overlaps', 'finishes'],
            },
            starts: {
                starts: ['starts'],
                during: ['during'],
                finishes: ['finishes', 'during', 'overlaps'],
            },
            during: {
                during: ['during'],
                finishes: ['finishes'],
        ...
        if (!triedInverse) {
            ...
            if (inv_r1 && inv_r2) {
                const inv_composed = this._composeTemporalRelations(
                    inv_r2,
                    inv_r1,
                    true
                );
                if (inv_composed) {
                    return inv_composed
                        .map((r) => this._getInverseTemporalRelation(r))
                        .filter((r) => r);
                }
            }
        }
