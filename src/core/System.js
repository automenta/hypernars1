export class System {
    ...
    step() {
        ...
        if (!event || event.budget.priority < this.nar.config.budgetThreshold)
            return false;

        this.nar.memoryManager.updateRelevance(
            event.target,
            'processing',
        ...
        if (
            this.nar.state.currentStep %
                this.nar.config.questionResolutionInterval ===
            0
        ) {
        ...
        if (
            this.nar.state.stepsSinceMaintenance >=
            this.nar.config.memoryMaintenanceInterval
        ) {
        ...
        if (Math.random() < this.nar.config.cleanupProbability) {
            for (const [id, cache] of this.nar.state.pathCache) {
                if (cache.size > this.nar.config.maxPathCacheSize) {
                    this.nar.state.pathCache.set(
                        id,
                        new Set(
                            [...cache].slice(
                                -this.nar.config.pathCacheTruncationSize
                            )
                        )
                    );
                }
            ...
            for (const [questionId, answers] of this.nar.state.index
                .questionCache) {
                if (answers.length > this.nar.config.maxQuestionCacheSize) {
                    this.nar.state.index.questionCache.set(
                        questionId,
                        answers.slice(
                            -this.nar.config.questionCacheTruncationSize
                        )
                    );
                }
            }
