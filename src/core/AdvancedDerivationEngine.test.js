...
describe('AdvancedDerivationEngine', () => {
    ...
    beforeEach(() => {
        ...
        mockNar.learningEngine.getRuleProductivityStats.mockReturnValue(
            new Map()
        );
    });
    ...
    describe('Rule Management', () => {
        ...
        it('should boost a rule success rate', () => {
            ...
            expect(engine.rules.get('BoostTest').successRate).toBe(
                0.5 * 0.8 + 1 * 0.2
            ); // 0.4 + 0.2 = 0.6
        });
        ...
        it('should penalize a rule success rate', () => {
            ...
            expect(engine.rules.get('PenalizeTest').successRate).toBe(
                0.5 * 0.8 + 0 * 0.2
            ); // 0.4
        });
    });
    ...
    describe('Derivation Logic', () => {
        it('should derive transitive inheritance (A->B, B->C => A->C)', () => {
            ...
            expect(mockNar.api.inheritance).toHaveBeenCalledWith(
                'A',
                'C',
                expect.any(Object)
            );
        ...
        it('should derive analogy (X~Y, X->P => Y->P)', () => {
            const h_sim = createMockHyperedge('h_sim', 'Similarity', [
                'X',
                'Y',
            ]);
            const h_inh = createMockHyperedge('h_inh', 'Inheritance', [
                'X',
                'P',
            ]);
        ...
        it('should derive implication (A=>B, A => B)', () => {
            const h_imp = createMockHyperedge('h_imp', 'Implication', [
                'A',
                'B',
            ]);
            ...
            expect(mockNar.propagation.propagate).toHaveBeenCalledWith(
        ...
        it('should derive equivalence (A<=>B => A=>B and B=>A)', () => {
            const h_equiv = createMockHyperedge('h_equiv', 'Equivalence', [
                'A',
                'B',
            ]);
            ...
            expect(mockNar.api.implication).toHaveBeenCalledWith(
                'A',
                'B',
                expect.any(Object)
            );
    ...
    describe('Temporal Relation Composition', () => {
        it('should compose "before" and "before" to "before"', () => {
            const result = engine._composeTemporalRelations('before', 'before');
            expect(result).toEqual(['before']);
        });

        it('should compose "meets" and "starts" to "starts"', () => {
            const result = engine._composeTemporalRelations('meets', 'starts');
            expect(result).toEqual(['starts']);
        });

        it('should compose "overlaps" and "during" to complex result', () => {
            const result = engine._composeTemporalRelations(
                'overlaps',
                'during'
            );
            expect(result).toEqual(
                expect.arrayContaining(['during', 'overlaps', 'finishes'])
            );
        });

        it('should return null for undefined compositions', () => {
            const result = engine._composeTemporalRelations('during', 'before');
            expect(result).toBeNull();
        });

        it('should use inverse relations for composition', () => {
            // Test A contains B, C before B. Should be A after C.
            // Inverse of contains is during. Inverse of before is after.
            // So, we are testing compose(after, during)
            // The table does not have `after`. So it should use inverse.
            // inv(after) = before, inv(during) = contains.
            // it should lookup compose(contains, before) which is not defined.
            // Let's try another one.
            // A after B, B after C => A after C
            // inv(after) = before. so compose(before, before) = before. inv(before) = after. Correct.
            const result = engine._composeTemporalRelations('after', 'after');
            expect(result).toEqual(['after']);
        });

        it('should derive transitive temporal relations', () => {
            // A before B, B meets C => A before C
            const h1 = createMockHyperedge('h1', 'TemporalRelation', [
                'A',
                'B',
                'before',
            ]);
            const h2 = createMockHyperedge('h2', 'TemporalRelation', [
                'B',
                'C',
                'meets',
            ]);
            ...
            engine._deriveTransitiveTemporalRelation(
                h2,
                event,
                'TemporalRelation'
            );
