export default {
    ...
    description: 'Tests the system\'s ability to represent and distinguish various forms of knowledge, including facts, rules, goals, and questions.',
    steps: [
        {
        ...
        {
            comment: 'Assert that the different knowledge types are correctly represented in the system\'s memory.',
            assert: (nar, logs) => {
                ...
                if (!uncertainBelief || Math.abs(uncertainBelief.truth.frequency - 0.8) > 0.01) {
                    return false;
                }
                ...
                if (!definitionBelief || definitionBelief.truth.isEternal() !== true) {
                    return false;
                }

                // Check for the procedural rule
                const ruleId = nar.implication(nar.set(nar.term('#x'), nar.term('animal')), nar.term('warm_blooded'));
                ...
                if (!nar.memory.questions.some(q => q.term.id === questionId)) {
                    return false;
                }
