...
describe('API Enhancements', () => {
    ...
    it('should get and resolve contradictions', () => {
        ...
        expect(contradictions1).toHaveLength(1);
    ...
    it('should resolve contradiction by specialization based on context', () => {
        ...
        expect(nar.getContradictions()).toHaveLength(0);
    ...
    it('should configure and get meta-reasoning strategy', () => {
        ...
        const strategy = nar.cognitiveExecutive.strategies.find(
            (s) => s.context === 'test_context'
        );
    ...
    it('should form a new concept from recurring patterns', () => {
        ...
        expect(conceptFormed.from).toEqual(
        ...
        const inheritanceLinks = nar.query(
            `<${conceptFormed.conceptId} --> $x>`
        );
        const inheritedTerms = inheritanceLinks.map(
            (link) => link.bindings['$x']
        );
        expect(inheritedTerms).toEqual(
            expect.arrayContaining([
                'Term(termA)',
                'Term(termB)',
                'Term(termC)',
