...
export class NAR extends EventEmitter {
        ...
        const moduleDefinitions = [
            {
                ...
                simple: ExpressionEvaluator,
                advanced: AdvancedExpressionEvaluator,
            },
            {
            ...
            {
                name: 'CognitiveExecutive',
                simple: CognitiveExecutive,
                advanced: CognitiveExecutive,
            },
            {
                name: 'ExplanationSystem',
                simple: ExplanationSystem,
                advanced: ExplanationSystem,
            },
            {
                name: 'GoalManager',
                simple: GoalManager,
                advanced: GoalManager,
                base: GoalManagerBase,
            },
            {
                name: 'ConceptFormation',
                simple: ConceptFormation,
                advanced: ConceptFormation,
            },
        ];

        for (const moduleDef of moduleDefinitions) {
            const instanceName =
                moduleDef.name.charAt(0).toLowerCase() +
                moduleDef.name.slice(1);
            const ModuleClass =
                customModules[moduleDef.name] ||
                (useAdvanced ? moduleDef.advanced : moduleDef.simple);
            ...
            if (ModuleClass) {
                ...
                if (
                    moduleDef.base &&
                    !(this[instanceName] instanceof moduleDef.base)
                ) {
                    throw new Error(
                        `Module ${instanceName} does not extend its base class correctly.`
                    );
                }
            }
        }
    ...
    _exposeApi() {
        ...
        this.explain = this.explanationSystem.explain.bind(
            this.explanationSystem
        );
        this.query = this.expressionEvaluator.query.bind(
    ...
    clearState() {
        ...
        if (
            ...
            this.contradictionManager.contradictions
        ) {
    ...
    saveState() {
        const hypergraphData = Array.from(this.state.hypergraph.values()).map(
            (h) => h.toJSON()
        );
        const stateData = {
            version: '1.0',
    ...
    loadState(jsonString) {
        ...
        for (const edgeData of stateData.hypergraph) {
            ...
            for (const beliefData of edgeData.beliefs) {
                const options = {
                    truth: new this.api.TruthValue(
                        beliefData.truth.frequency,
                        beliefData.truth.confidence,
                        beliefData.truth.priority
                    ),
                    budget: new this.api.Budget(
                        beliefData.budget.priority,
                        beliefData.budget.durability,
                        beliefData.budget.quality
                    ),
    ...
    _log(level, message, details = {}) {
        ...
        if (messageLevel >= currentLevel) {
            ...
            const logMethod =
                this.config.logger[level.toLowerCase()] ||
