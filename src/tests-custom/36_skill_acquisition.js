export default {
    ...
    description:
        'Tests if the system can learn a new procedural rule (a skill) to achieve a goal more efficiently.',
    steps: [
        {
            ...
            action: (nar) => {
                ...
                nar.nal(
                    '(((*, have_flour, have_sugar, have_eggs) ==> can_bake_cake)). %1.0; 0.9%'
                );
        ...
        {
            comment:
                'Introduce a shortcut and see if a new, more efficient rule is learned.',
            action: (nar) => {
                // A cake mix provides flour and sugar
                nar.nal(
                    '((*_ have_cake_mix) ==> (*, have_flour, have_sugar)). %1.0; 0.9%'
                );
        ...
        {
            comment:
                'Assert that the new, more efficient skill has been learned.',
            assert: (nar, logs) => {
                // The new skill/rule to check for
                const newRuleTerm = nar.implication(
                    nar.conjunction(
                        nar.term('have_cake_mix'),
                        nar.term('have_eggs'),
                        nar.term('*_')
                    ),
                ...
                return (
                    newRuleConcept &&
                    newRuleConcept.beliefs.some(
                        (b) => b.truth.expectation() > 0.7
                    )
                );
            },
        },
