export default {
    ...
    steps: [
        {
            comment:
                'Step 1: Provide the necessary background knowledge for the task.',
            action: (nar) => {
        ...
        },
        {
            ...
            assert: (nar, logs) => {
                return nar.goalManager
                    .getActiveGoals()
                    .some((g) => g.description === 'open(door)');
            },
        },
        {
            comment:
                'Step 3: Run the system and assert that it achieves the goal through sub-goaling.',
            action: (nar) => {
            ...
            assert: (nar, logs) => {
                ...
                if (!goalHyperedgeId) {
                    logs.push(
                        '[ASSERT FAILED] The goal "open(door)" was not found in the knowledge base.'
                    );
                    return false;
                }
                ...
                if (!belief || belief.truth.expectation() < 0.7) {
                    logs.push(
                        `[ASSERT FAILED] Belief in "open(door)" is too low. Expectation: ${belief?.truth.expectation()}`
                    );
                    return false;
                }
                return true;
