export default {
    ...
    description:
        'Tests the ability to resolve the meaning of an ambiguous term using context.',
    steps: [
        {
            comment:
                'Establish background knowledge about the two meanings of "bank".',
            action: (nar) => {
                // Meaning 1: A bank is a financial institution.
                nar.nal('<bank_financial ==> bank>. %1.0;0.9%');
                nar.nal(
                    '<bank_financial ==> has_property_deposits>. %1.0;0.9%'
                );
        ...
        {
            comment:
                'Provide a sentence with the ambiguous term in a financial context.',
            action: (nar) => {
                ...
                nar.nal(
                    '<involves_deposits ==> has_property_deposits>. %1.0;0.9%'
                );
            ...
            assert: (nar, logs) => {
                ...
                const belief = nar.getBelief(
                    '<event_deposits ==> bank_financial>'
                );

                // And it should not associate it with the river bank.
                const mistakenBelief = nar.getBelief(
                    '<event_deposits ==> bank_river>'
                );

                return (
                    belief &&
                    belief.truth.expectation() > 0.6 &&
                    (!mistakenBelief ||
                        mistakenBelief.truth.expectation() < 0.3)
                );
            },
        },
        {
            comment:
                'Provide a sentence with the ambiguous term in a nature context.',
            action: (nar) => {
                ...
                nar.nal(
                    '<involves_fishing ==> has_property_fishing>. %1.0;0.9%'
                );
