export default {
    ...
    description:
        "Tests the system's ability to transfer knowledge between structurally similar concepts.",
    steps: [
        {
            comment:
                'Establish a relationship in a base domain (solar system) and an analogous one in a target domain (atom), then check for knowledge transfer.',
            action: (nar) => {
                // Base domain knowledge: A system with a central star, has a central body.
                // Using simplified NAL for clarity.
                nar.nal(
                    '((*, {star}, {planet}) ==> has_central_body). %1.0;0.9%'
                );
            ...
            assert: (nar, logs) => {
                ...
                const inferredBelief = nar.getBelief(
                    '<atom ==> has_central_body>'
                );

                // The belief should be present and have a high expectation.
                return (
                    inferredBelief && inferredBelief.truth.expectation() > 0.5
                );
            },
        },
    ],
