export default [
    {
        ...
        description:
            'Tests the interaction between the learning engine and memory manager by learning a fact, forgetting it over time, and then re-learning it.',
        skipped: false,
        steps: [
            {
                ...
                assert: (nar, logs) => {
                    const beliefId = nar.inheritance(
                        'fact_to_forget',
                        'has_property'
                    );
                    ...
                    if (!belief) {
                        console.error(
                            'Assertion failed: Belief was not created.'
                        );
                        return false;
                    }
            ...
            {
                comment:
                    'Step 2: Run the system for many cycles to induce forgetting.',
                action: (nar) => {
                ...
                assert: (nar, logs) => {
                    ...
                    const currentBudget = hyperedge
                        .getStrongestBelief()
                        .budget.total();
                    const wasForgotten =
                        currentBudget < nar.scratchpad.initialBudget * 0.1; // e.g., less than 10% of initial budget
                    if (!wasForgotten) {
                        console.error(
                            `Assertion failed: Belief was not forgotten. Initial budget: ${nar.scratchpad.initialBudget}, Current budget: ${currentBudget}`
                        );
                    }
                    return wasForgotten;
                },
            ...
            {
                comment:
                    'Step 3: Re-introduce the fact and verify it is re-learned.',
                action: (nar) => {
                    nar.nal('(<fact_to_forget --> has_property>). %1.0;0.9%');
                    nar.run(10);
                },
                assert: (nar, logs) => {
                    ...
                    if (!belief) {
                        console.error(
                            'Assertion failed: Belief was not re-learned.'
                        );
                        return false;
                    }
    ...
    },
    {
        ...
        description:
            'This test is designed to fail. It checks if a goal is pursued even if its prerequisite is backed by a belief with very low, fluctuating confidence.',
        expectedResult: false,
        steps: [
            {
                comment:
                    'Establish a very unstable belief that action_X leads to goal_Y.',
                action: (nar) => {
            ...
            },
            {
                ...
                assert: (nar, logs) => {
                    ...
                    const actionXGoal = goals.find(
                        (g) => g.description === 'action_X'
                    );
                    if (!actionXGoal) {
                        console.error(
                            'Failing test assertion failed: Subgoal for action_X was not created.'
                        );
                    ...
                    if (!isHighPriority) {
                        console.error(
                            `Failing test assertion failed: Subgoal for action_X has low priority (${actionXGoal.utility}), which is the desired behavior. The test is designed to fail if the priority is high.`
                        );
                    }
