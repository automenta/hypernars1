export default {
    ...
    description: 'Tests if the system can model the beliefs of other agents, even when those beliefs are false.',
    steps: [
        {
            comment: 'Setup: Sally puts her marble in the box. The system knows this and knows that Sally believes it.',
            action: (nar) => {
        ...
        {
            comment: 'The state of the world changes while Sally is not present. Anne moves the marble.',
            action: (nar) => {
        ...
        },
        {
            ...
            assert: (nar, logs) => {
                ...
                const reportsFalseBelief = answerLog.includes('<marble --> in_box>');
                const doesNotReportTrueState = !answerLog.includes('<marble --> in_basket>');
