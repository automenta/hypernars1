export default {
    ...
    description: 'Tests the system\'s ability to forget irrelevant or outdated information over time.',
    ...
    steps: [
        {
            comment: 'Introduce two facts, one normal and one important (higher budget).',
            action: (nar) => {
                // An ordinary fact
            ...
            assert: (nar, logs) => {
                ...
                const importantConcept = nar.getConcept(nar.term('important_fact').id);
                return oldConcept && importantConcept;
            }
        },
        {
            comment: 'Flood the system with a large amount of new, unrelated information.',
            action: (nar) => {
                for (let i = 0; i < 200; i++) {
        ...
        {
            comment: 'Assert that the less important fact was forgotten while the important one remains.',
            assert: (nar, logs) => {
                const oldConcept = nar.getConcept(nar.term('old_fact').id);
                const importantConcept = nar.getConcept(nar.term('important_fact').id);

                // The old concept should have a very low budget (be forgotten) or be gone entirely.
                const isForgotten = !oldConcept || oldConcept.budget.priority < 0.01;
                // The important concept should still have a reasonably high budget.
                const isRemembered = importantConcept && importantConcept.budget.priority > 0.5;
