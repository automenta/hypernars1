export default {
    ...
    description:
        'Tests if the system revises its beliefs when its actions lead to goal failure.',
    steps: [
        {
            comment:
                'Setup: a goal, a flawed plan, and a condition that causes the plan to fail.',
            action: (nar) => {
                ...
                nar.nal(
                    '<(&&, flip_switch, no_power) =/> light_off>. %1.0;0.9%'
                );
            ...
            assert: (nar, logs) => {
                ...
                const flawedBelief = nar.getBelief(
                    '<<flip_switch =/> light_on>>'
                );
        ...
        {
            comment:
                'Introduce a correct plan and verify the system adopts it.',
            action: (nar) => {
                ...
                nar.nal(
                    '<(&&, flip_switch, power_on) =/> light_on>. %1.0;0.9%'
                );
            ...
            assert: (nar, logs) => {
                ...
                const hasNewGoal = logs.some((log) =>
                    log.includes('<turn_on_generator!>')
                );
                return hasNewGoal;
            },
        },
