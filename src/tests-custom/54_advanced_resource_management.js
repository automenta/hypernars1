    ...
    steps: [
        {
            ...
            assert: (nar) => {
                ...
                const derivationTask = {
                    type: 'derivation',
                    args: ['(a --> b).', '(b --> c).'],
                };

                const questionBudget =
                    nar._resources.allocateResources(questionTask);
                const derivationBudget =
                    nar._resources.allocateResources(derivationTask);
        ...
        },
        {
            ...
            assert: (nar) => {
                ...
                const urgentTask = {
                    type: 'critical-event',
                    args: ['(reactor --> meltdown).'],
                };

                const normalContext = {};
                const urgentContext = { urgency: 0.9, importance: 0.95 };

                const normalBudget = nar._resources.allocateResources(
                    normalTask,
                    normalContext
                );
                const urgentBudget = nar._resources.allocateResources(
                    urgentTask,
                    urgentContext
                );

                return (
                    urgentBudget.priority > normalBudget.priority &&
                    urgentBudget.durability > normalBudget.durability
                );
            },
        },
        {
            ...
            action: (nar) => {
                ...
                nar.eventQueue.add({
                    name: 'low_value_event_1',
                    budget: new Budget(0.1, 0.1, 0.1),
                });
                nar.eventQueue.add({
                    name: 'low_value_event_2',
                    budget: new Budget(0.15, 0.1, 0.1),
                });
            },
            assert: (nar) => {
                ...
                return (
                    prunedCount === 2 &&
                    queueSize === 1 &&
                    nar.eventQueue.peek().name === 'high_value_event'
                );
            },
        },
    ],
