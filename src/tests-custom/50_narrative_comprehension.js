export default {
    ...
    description:
        'Tests the ability to follow a simple story and track state changes over time.',
    steps: [
        {
            comment:
                'Introduce the initial state and the "physics" of the story world.',
            action: (nar) => {
                ...
                nar.nal(
                    '<(<#x --> food> & <(*, $y, #x) =/> eats>) =/> <$y =/> !is_hungry>>. %1.0;0.9%'
                );
        ...
        {
            comment:
                'Present the key event of the narrative: John eats an apple.',
            action: (nar) => {
