export default {
    ...
    description:
        'Tests system performance and stability with a large knowledge base.',
    skipped: false, // Skipping due to deeper bug in reasoning engine.
    steps: [
        {
            comment:
                'Step 1: Populate the knowledge base with a large number of beliefs and rules.',
            action: (nar) => {
        ...
        {
            comment:
                'Step 2: Run the system for enough cycles to process the knowledge.',
            action: (nar) => {
                nar.run(200);
            },
        },
        {
            comment:
                'Step 3: Perform a query and assert the result is found correctly.',
            action: (nar) => {
            ...
            assert: (nar, logs) => {
                ...
                if (!result || result.length === 0) {
                    logs.push(
                        '[ASSERT FAILED] Query for target property returned no results.'
                    );
                    return false;
                }
                ...
                if (binding !== 'target_property') {
                    logs.push(
                        `[ASSERT FAILED] Query returned wrong binding. Expected "target_property", got "${binding}".`
                    );
                    return false;
                }
