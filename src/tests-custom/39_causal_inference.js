export default {
    ...
    description:
        "Tests the system's ability to distinguish true causal factors from mere correlations.",
    steps: [
        {
            comment:
                'Provide evidence that A and B are individually correlated with C, but are only jointly sufficient to cause C.',
            action: (nar) => {
                ...
                for (let i = 0; i < 10; i++) {
                    ...
                    nar.nal(
                        '<(<{switch_on} & <#power_on>>) =/> <#light_on>>. %1.0;0.9%'
                    );
                    // When switch is flipped but power is off, light is off.
                    nar.nal(
                        '<(<{switch_on} & <#power_off>>) =/> <#light_off>>. %1.0;0.9%'
                    );
                    // When switch is not flipped but power is on, light is off.
                    nar.nal(
                        '<(<{switch_off} & <#power_on>>) =/> <#light_off>>. %1.0;0.9%'
                    );
                }
                nar.run(200);
            },
            assert: (nar, logs) => {
                ...
                if (
                    !causalLinkBelief ||
                    causalLinkBelief.truth.confidence < 0.8
                ) {
                    return false;
                }

                // The simple correlations should be significantly weaker than the true causal link.
                const switchConfidence = switchCorrelationBelief
                    ? switchCorrelationBelief.truth.confidence
                    : 0;
                const powerConfidence = powerCorrelationBelief
                    ? powerCorrelationBelief.truth.confidence
                    : 0;

                return (
                    switchConfidence < causalLinkBelief.truth.confidence &&
