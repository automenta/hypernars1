export default {
    ...
    description:
        'Tests advanced inference forms like abduction (inference to the best explanation) and induction (generalization).',
    steps: [
        {
            comment: 'Setup for abduction: provide a rule and an observation.',
            action: (nar) => {
                nar.nal(
                    '((grass_is_wet ==> (sprinkler_was_on || it_rained))). %0.9; 0.9%'
                );
        ...
        {
            comment:
                'Assert abduction: check for beliefs about possible explanations.',
            assert: (nar, logs) => {
                const explanationId = nar.disjunction(
                    nar.term('sprinkler_was_on'),
                    nar.term('it_rained')
                );
